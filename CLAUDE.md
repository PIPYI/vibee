...
아직 답이 정해지지 않은 문제에 **선택지를 들이밀지 않는다.** 선택지는 논의를 너무 이르게
좁힌다. 근거와 추천을 산문으로 제시하고 열린 질문 하나로 되묻는다.

선택지를 물어도 되는 것은 **되돌리기 어려운 운영 결정**뿐이다 — 브랜치 정리, push 여부,
설치처럼 이미 답이 갈려 있는 경우.

방향이 바뀌면 재논의하지 말고 바로 반영한다.

### 문서는 새로 만들지 않는다

정리할 내용이 생기면 **이미 있는 문서에** 넣는다. 사본이 늘어나면 어느 것이 최신인지
알 수 없게 된다.

| 무엇 | 어디 |
| --- | --- |
| 제품 설계 | `docs/` |
| 검증 결과 · 함정 | `SPIKE_FINDINGS.md` |
| 실행 방법 | 각 `README.md` |
| 작업 규칙 | 이 파일 |
| 현재 상태 | 루트 `README.md`의 "상태" 절 |

같은 이유로 harness에는 규율만 넣는다. "무엇을 만드는가"는 설계 문서가 말한다
(`docs/requirements_flow.md` §6).

**프로토타입은 검증용이지 제품이 아니다.** 루트 프로젝트처럼 문서를 갖출 필요가 없다.
