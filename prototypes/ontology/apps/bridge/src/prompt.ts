...
const ASSEMBLY_RULES = [
  ...
  "2. architecture.connections 는 confirmed|grounded이면서 valid|relocated인 System Link의",
  "   연속된 방향 경로를 요약해야 한다(I20-v4). inferred/needs_review Link는 사용할 수 없다.",
  "   workflow.edges 는 대신 evidenceRefs 로만 근거를 댄다",
...
export function buildAssemblyPrompt(projectPath: string, skeletonSummary: string, topologySummary: string): string {
  return [
    ...
    "순서:",
    "1. get_project_semantic_memory 로 Concept·Scenario 전체를 훑는다.",
    "2. get_system_facts로 검증된 System Entity와 System Link ID를 가져온다. 이어서",
    "   CanonicalScenario와 중요한 Concept anchor가 둘 이상이면 get_impact_context_batch로",
    ...
    "5. architecture.connections 를 만든다 — systemLinkRefs에 2번에서 확인한 System Link ID를",
