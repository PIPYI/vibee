...
### bridge를 다시 켜야 하는데 "포트가 이미 쓰이고 있다"고 나올 때

`npm run bridge`는 `npm run start -w @onto/bridge` → `node dist/index.js`로 두 겹
감싸여 있다. 이 중첩 때문에 터미널에서 Ctrl+C를 눌러도 가장 안쪽 `node` 프로세스까지
신호가 전달되지 않는 경우가 있다(npm의 알려진 한계) — 그러면 이전 bridge가 고아
프로세스로 남아 포트를 계속 물고 있다.

```bash
npm run bridge:stop
```

포트에 떠 있는 것이 **정말 우리 bridge인지 확인한 뒤에만** 정리한다(`/api/state`에
우리가 아는 모양으로 응답하는지 본다) — 확인되지 않으면 죽이지 않고 PID만 알려주고
멈춘다. 이미 꺼져 있으면 아무 일도 하지 않는다(몇 번을 돌려도 안전하다).
