...
무언가를 찾아낸다. 그래서 무해 케이스에는 **일반 코드 리뷰거리**(중복 로직, 잘라낸 매직 넘버,
...
`npm run drift` — 케이스당 4회(첫 확인 1 + 반복 3), 두 provider.

| provider | 모델 | 검출 | 오탐 없음 |
| --- | --- | --- | --- |
| codex | `gpt-5.6-luna` (effort `low`) | 4/4 | 4/4 |
| claude | `haiku` | 4/4 | 4/4 |
...
- 위반 케이스에서 `DEC-1`을 짚고 `payment.js` / `rental.js`를 지목했다
- **리뷰 turn이 파일을 하나도 건드리지 않았다** (`git status --porcelain`이 비어 있음)
...
- **리뷰는 정의상 인계가 끝나고 한참 뒤에 돈다.** 그때 bridge는 이미 재시작됐다. §13의
  ...
  리뷰 자체가 시작되지 않는다.
- **diff를 조용히 자르면 안 된다.** `MAX_DIFF_CHARS`를 넘으면 자르되 `truncated: true`를
  함께 넘긴다. 잘린 줄 모르면 agent가 "diff에 없으니 위반이 없다"고 판단한다.
- **무해 케이스를 위반 커밋 위에 쌓지 않는다.** 결제 코드가 저장소에 남아 있으면, 리뷰어가
  diff 밖의 것을 보고 짚었을 때 오탐인지 정당한 지적인지 가릴 수 없다. 케이스마다 fixture를
  새로 만든다.
...
대신 **고치기 시작한다.** fixture에 하네스를 일부러 놓아 두고 확인했다 — 워킹 트리는
매번 깨끗했다.
...
- **diff만 본다.** 현재 파일 전체를 읽지 않으므로, 위반이 여러 커밋에 걸쳐 나뉘어 들어온
  경우(import는 3커밋 전, 호출부는 지금)는 잡지 못한다. 첫 스캔을 코드 전체에 대고 하는
  경로도 아직 없다.
- **기준이 8개일 때의 결과다.** 결정이 30개 쌓였을 때 정확도가 유지되는지는 재지 않았다.
- **위반이 하나뿐인 diff다.** 둘 이상이 섞였을 때 전부 짚는지는 재지 않았다.
- **해소 경로가 없다.** 검출만 있고, 어긋난 것을 고칠 프롬프트를 렌더하거나 DEC을 갱신하는
  경로는 만들지 않았다. 리포트도 아직 수명(열림 → 승인됨 / 수정됨 / 무시됨)을 갖지 않아서
  같은 드리프트가 매번 다시 뜬다.
