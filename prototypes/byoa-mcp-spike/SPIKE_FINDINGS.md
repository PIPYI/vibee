...
처음 만드는 프로젝트에는 `app_design.md`도 하네스도 없고, 산출물은 [4] 인계에서만 디스크에
쓰인다. 그래서 **정상 흐름에서는 인터뷰가 하네스를 마주치지 않는다.** 이 spike에서 실제로
오염이 일어난 것은 지난 실행의 산출물이 fixture에 남아 있었기 때문이다.

그래도 격리를 두는 이유는 셋이다 (`docs/requirements_flow.md` §6).

- **빈 폴더가 아닐 수 있다.** 사용자나 다른 도구가 만든 `CLAUDE.md` · `AGENTS.md`에는 우리
  마커가 없으므로 지우지 않는다. 지우지 않으면 인터뷰 turn에 실린다.
- **지우기가 실패할 수 있다.** 권한 문제로 정리에 실패해도 인터뷰는 오염되지 않아야 한다.
- **인터뷰에는 도구가 필요 없다.** 하네스가 없어도 파일 읽기·셸·하위 에이전트 도구 정의가
  매 turn 실린다.

그리고 **인터뷰를 새로 시작하는 것은 새 프로젝트를 시작하는 것이다.** 3.1은 프로젝트당 한 번
도는 기능이고, 인계 이후의 수정은 3.3 drift가 맡는다. 같은 디렉터리에서 다시 시작했다면
만들려는 앱이 달라진 것이므로, `POST /api/tasks`가 `mode: "interview"`로 들어오면 지난
흔적을 세 곳에서 모두 지운다 — bridge의 문답·초안, agent 쪽 세션, 디스크의 산출물.
