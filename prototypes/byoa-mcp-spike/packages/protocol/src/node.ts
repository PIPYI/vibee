/**
 * Node 전용 헬퍼. spike 체크아웃 위치를 찾고, bridge · MCP server · 등록 스크립트가
...
const ROOT_PACKAGE_NAME = "byoa-mcp-spike";

/** `startPath`에서 위로 올라가며 spike의 루트 package.json을 찾는다. */
export function findSpikeRoot(startPath: string): string {
...
export function spikeRootFromModule(moduleUrl: string): string {
  return findSpikeRoot(dirname(fileURLToPath(moduleUrl)));
}
 ...
 * 정의는 여기 한 곳에만 둔다.
 */
export function fixturePath(spikeRoot: string): string {
  return join(spikeRoot, "tmp", "fixture");
}
...
/**
 * `.byoa-spike/bridge.json`을 읽고, 처음 쓸 때는 새 토큰과 함께 생성한다.
 * bridge와 등록 스크립트가 모두 이 함수를 거치므로, MCP server에 서로 맞는
 * BRIDGE_URL/BRIDGE_TOKEN 쌍을 넘겨줄 수 있다.
 */
export function loadBridgeConfig(spikeRoot: string): BridgeConfig {
  const dir = join(spikeRoot, ".byoa-spike");
  const configPath = join(dir, "bridge.json");

  let port = Number(process.env.BRIDGE_PORT ?? DEFAULT_BRIDGE_PORT);
  let token: string | undefined;

  if (existsSync(configPath)) {
    const saved = JSON.parse(readFileSync(configPath, "utf8")) as { port?: number; token?: string };
    if (!process.env.BRIDGE_PORT && typeof saved.port === "number") port = saved.port;
    token = saved.token;
  }
