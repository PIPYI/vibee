...
const ALL_MODES: readonly TaskMode[] = ["interview", "review", "wiki", "architecture", "analyze", "assembly"];
  ...
  async startTask(input: StartTaskInput, emit: (event: AgentEvent) => void): Promise<TaskOutcome> {
    ...
    const stream = query({
      ...
      options: {
        ...
        mcpServers,
        ...(needsReadTools(input.mode) ? { tools: [...READ_ONLY_TOOLS] } : { tools: [] }),
        // CLAUDE.md/AGENTS.md는 [4] 인계 산출물이지 이 앱의 어떤 turn의 규칙도 아니다.
        // 이 앱에는 코드를 쓰는 mode가 없으므로 모든 turn에서 항상 끈다.
        settingSources: [],
