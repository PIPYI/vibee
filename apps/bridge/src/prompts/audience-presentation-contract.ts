...
- **Every visible simple-view text must be Korean.** Write the document title, every visible simple component/boundary/connection label and sublabel, and every card title/item in Korean. Proper names or standard acronyms may remain only inside a Korean phrase (for example, "OpenAI 답변 서비스" or "API 요청 처리"); never leave an English-only phrase such as "Store Uploaded Documents" in the simple view. Add a \`presentation.simple\` override whenever the canonical technical text is not Korean.
