...
| **Phase E** (드리프트 검출 · 해소) | 인터뷰가 저장한 DEC/RULE이 **코드 변경을 판정하는 기준으로** 쓰인다. 한 리뷰에 커밋 둘을 넣으면 여덟 기준 중 깨진 하나를 위반 커밋에서만 짚고, 리뷰거리를 심어 둔 무해 커밋은 건드리지 않는다 (4/4). 그 finding을 고칠 프롬프트를 렌더해 사용자의 옆 agent에게 넘기면, 코드와 결정 중 **정확히 한쪽만** 고쳐진다 — 두 provider 3/3 |
| **Phase F** (위키) | 바이브코딩 대화에서 비전공자가 모를 만한 말을 agent가 **판단으로** 골라낸다. 빈도로 뽑으려던 최초 시도는 완전히 실패했다 — 실제 대화에서 최다 빈출어는 전부 노이즈였다. 판단으로 바꾸자 한글 용어까지 잡았고, 페이지는 실제 코드 경로를 근거로 대며 평가하지 않는다 — 두 provider 13/13 |
...
Phase E로 "Intent ↔ Code" 한 바퀴가 닫혔다 — 인터뷰가 정한 것이 코드를 판정하고, 어긋난
것은 코드나 결정 중 한쪽을 고치는 프롬프트로 되돌아간다. 다만 **결정을 갱신하는 분기는
문구만 확인했지 실측하지 않았다** — 검증에 쓴 위반이 명백해서 두 provider 모두 코드를
고치는 쪽만 골랐다 (`SPIKE_FINDINGS.md` §15).

그리고 지금 드리프트 검증의 입력은 **손으로 쓴 커밋**이다. 인터뷰가 만든 설계로 실제 앱이
만들어지고 그 코드에서 드리프트가 나오는지, 해소 프롬프트를 실제로 옆 agent 세션(우리
bridge가 흉내낸 것이 아니라 진짜 별도 창)에 붙여넣었을 때도 같은지는 아직 보지 않았다.

Phase F가 남긴 것은 커버리지다. 평가 표현을 거르는 필터(`findAdvice`)가 정규식 목록이라
완전하지 않고, 여러 후보 키워드 중 하나로만 검증했다 (`SPIKE_FINDINGS.md` §16).

기능 3.1의 남은 검증 항목은 [`docs/requirements_flow.md`](./docs/requirements_flow.md) §8에 있다.
그중 **인터뷰 상태의 지속성**은 리뷰 경로에서 이미 절반 풀렸다 — 리뷰는 디스크의
`.project-intel/design.json`을 읽는다. 화면의 문답 기록은 여전히 메모리에만 있다.
