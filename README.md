...
| **Phase E** (드리프트 검출) | 인터뷰가 저장한 DEC/RULE이 **코드 변경을 판정하는 기준으로** 쓰인다. 여덟 기준 중 깨진 하나를 짚어내고, 아무것도 어기지 않은 변경에서는 **조용히 있는다** — 두 provider 4/4 · 4/4 |
...
**해소**다. 지금은 검출만 하고 리포트가 수명을 갖지 않아서 같은 드리프트가 매번 다시 뜬다.
어긋난 것을 발견했을 때 코드를 고칠 프롬프트를 건네거나 결정을 갱신하는 경로가 있어야
"**지속** 동기화"가 된다 (`SPIKE_FINDINGS.md` §15 "이것이 확인해 주지 않는 것").
