...
| **Phase E** (드리프트 검출) | 인터뷰가 저장한 DEC/RULE이 **코드 변경을 판정하는 기준으로** 쓰인다. 한 리뷰에 커밋 둘을 넣으면 여덟 기준 중 깨진 하나를 위반 커밋에서만 짚고, 리뷰거리를 심어 둔 무해 커밋은 건드리지 않는다 — 두 provider 4/4 |
...
Phase E로 한 바퀴의 절반이 닫혔다 — 인터뷰가 정한 것이 나중에 코드를 판정한다. 나머지 절반은
**해소**다. 어긋난 것을 발견했을 때 코드를 고칠 프롬프트를 건네거나, 사용자가 마음을 바꾼
경우 결정 자체를 갱신하는 경로가 있어야 "**지속** 동기화"가 된다.

그리고 지금 드리프트 검증의 입력은 **손으로 쓴 커밋**이다. 인터뷰가 만든 설계로 실제 앱이
만들어지고 그 코드에서 드리프트가 나오는지는 아직 보지 않았다 — 한 바퀴를 통째로 돌리는
것은 해소 경로가 생긴 뒤가 맞다 (`SPIKE_FINDINGS.md` §15 "이것이 확인해 주지 않는 것").
