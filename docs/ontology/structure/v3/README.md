...
- 현재 구현 기준: **V3.1** (`ac10424`, 커밋 제목은 실수로 `V2.4`)
- 다음 대상: **V3.2** 분석 런타임 신뢰성·비용·진행 UX 개편
...
| 공식 버전 | Git 기준 | 설명 |
|---|---|---|
| V3.0 | `b3d439f` (`V3`) | 통합 지도와 Stage 분리의 첫 구현 |
| V3.1 | `ac10424` (커밋 제목 `V2.4`) | 단일 여정 캔버스와 핵심 관계 UX |
| V3.2 | 구현 중 | 분석 계약·재시도·토큰·대기 UX·Python 근거 개선 |

이미 공유된 Git 이력을 다시 쓰지 않고 문서와 이후 릴리스 노트에서 `ac10424 = V3.1`로
교정한다. V3.2의 구체적인 범위와 완료 조건은 [V3.2 구현 계획](./v3.2_plan.md)을 따른다.
...
3. [V3.2 구현 계획](./v3.2_plan.md): V3.1 진단 사례를 기준으로 분석 계약, bounded correction,
   진행 UX, Python Evidence를 구현한다.
...
분석 런타임과 비용·대기 UX 관련 남은 항목은 V3.2 범위로 승격했다. 구현 순서와 수용 기준은
[V3.2 구현 계획](./v3.2_plan.md)을 source of truth로 사용한다.
