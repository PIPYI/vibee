...
> V3.1 구현 메모(2026-08-24): 탭 없는 통합 정보 구조, `핵심 관계`, compact journey selector,
> 단일 스크롤 여정 캔버스, DOM 실측 SVG 분기선, exact ref 기반 양방향 강조를 구현했다.
> 고급 lane 충돌 회피, 필터, URL focus, modal focus trap은 후속 작업이다. 세부 상태는
> [README V3.1 기준선](./README.md#v31-구현-기준선--2026-08-24)을 따른다. V3.2에서는
> 이 화면의 대규모 재설계보다 [분석 런타임 신뢰성과 진행 UX](./v3.2_plan.md)를 우선한다.
...
시스템 구조와 사용자 여정은 별도 탭이나 sticky anchor 없이 문서 흐름으로 이어진다. 분석
범위와 최신성은 메인 지도의 고정 strip이 아니라 분석 상태·진단에서 필요할 때 확인한다.
...
| 분석 범위 확인 | 메인 strip 폐기, 분석 상태·진단으로 이동 | 지도 읽기를 방해하지 않되 데이터는 보존 |
| 먼저 읽을 흐름 | 별도 섹션 폐기 | 실제 흐름·사용자 여정으로 오해 가능 |
| `viewPlan.primaryPath` | `핵심 관계` 강조에 재사용 | 비전공자 온보딩 기능은 유지 |
...
- `핵심 관계`를 켜면 primaryPath 노드에 `추천 01`, `추천 02` 순번을 붙인다.
...
4. RepositoryTopology coverage 데이터는 유지되지만 메인 지도에 고정 strip으로 노출하지 않는다.
5. `먼저 읽을 흐름`은 별도 섹션이 아니며 `핵심 관계` 추천 순서로만 사용된다.
