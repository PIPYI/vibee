...
발견했을 때 무엇을 고칠지 정하는 경로 — §13의 `[기존 결정 유지] / [현재 구현 승인] /
[새 결정으로 교체]` — 가 붙어야 "지속 동기화"가 된다. 바이브코딩에서는 **결정을 갱신하는
쪽이 더 흔할 것**이므로 기본 동작을 거기 맞춘다.

결정을 갱신한다는 것은 `.project-intel/`의 DEC을 고치는 일이고, 그것은 §3.1 인터뷰의
연장이다. 두 기능이 별개가 아니라 한 바퀴다.

> 검증: `SPIKE_FINDINGS.md` §15. 두 provider에서 검출 4/4 · 오탐 없음 4/4.
...
### 3.5 Bookmark / Wiki / Project Memory

바이브코딩 대화 중 중요한 내용을 저장한다.
