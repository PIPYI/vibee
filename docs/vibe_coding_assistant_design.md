...
### 기능은 독립적이다 — 진입 경로와 실행 모델

3.1~3.5는 하나로 이어지는 파이프라인이 아니라 **서로 다른 목적을 가진 별개 기능**이다.
같은 프로젝트 디렉터리를 워크스페이스로 공유하므로 필요하면 서로의 산출물(설계, 코드,
대화, git 이력)을 읽을 수 있지만, 그렇다고 하나의 세션·상태·갱신 주기를 공유하지는
않는다. **한 기능을 실행했다고 다른 기능의 산출물이 초기화되거나 무효화되면 안 된다.**

두 진입 경로가 있다.

- **새 앱을 만드는 경우** — 새 프로젝트 디렉터리 지정 → 3.1 인터뷰 → 설계도·harness 인계
  → 같은 디렉터리에서 사용자의 Codex/Claude Code가 결과물을 끝까지 구현 → 이후 후속
  기능(3.2~3.5)을 필요할 때 개별 실행한다.
- **이미 코드가 있는 프로젝트를 여는 경우** — 인터뷰를 거치지 않고, 코드 분석·시각화나
  아키텍처·기술부채 같은 기능부터 바로 실행한다. 이 경로에서 3.3(Drift)이 무엇을 기준으로
  삼을지는 아직 정하지 않았다 — DEC/RULE 자체가 없는 저장소이기 때문이다.

**"완성된 프로젝트가 다시 우리 앱으로 돌아온다"는 표현은 import나 재연결이 아니다.**
프로젝트 경로는 이미 앱에 연결돼 있고, Codex/Claude Code와 우리 앱은 처음부터 같은
디렉터리를 본다. 사용자가 후속 기능을 실행하면 그 시점에 시작된 turn이 **현재
워크스페이스를 새로 읽을 뿐**이며, 별도의 업로드·재등록 단계는 없다. 우리 앱이 외부
에이전트의 "완료"를 자동 감지해야 한다는 요구도 없다 — 중요한 것은 완료 이후 사용자가
기능을 누른다는 흐름이지, 자동 상태 판정이 아니다.

기능마다 필요한 입력과 코드를 쓰는 주체가 다르다.

| 기능 | 주된 입력 | 코드/설계를 실제로 고치는 주체 |
| --- | --- | --- |
| 3.1 인터뷰 | 새 프로젝트에 대한 사용자 대화 | — (설계 산출물만 만든다) |
| 3.2 시각화 | 실행 시점의 프로젝트 코드 전체 | 사용자의 Codex/Claude Code |
| 3.3 Drift | DEC/RULE + 아직 검토하지 않은 커밋 diff | 사용자의 Codex/Claude Code (해소 프롬프트로) |
| 3.4 기술부채 | 기존 코드베이스 + 프로젝트 기록 | 사용자의 Codex/Claude Code |
| 3.5 Wiki | 완성 과정의 에이전트 채팅 기록 + 근거 확인용 코드 | — (읽기만 한다) |

**우리 앱은 소스 코드를 쓰지 않는다.** 3.1은 설계 산출물을, 3.3은 고칠 프롬프트를 만들
뿐이고, 실제 코드/설계 변경은 항상 사용자가 연결한 Codex 또는 Claude Code가 한다
(`docs/BYOA_MCP_INTEGRATION_SPIKE.md` §1.2). spike의 `mode: "task"`가 파일을 직접
수정하는 것은 Browser→Agent 제어 채널이 실제로 동작하는지 확인한 acceptance 장치이지,
제품의 내장 코딩 화면으로 확장할 것이 아니다.

**인터뷰는 인계로 끝난다.** 인계 후 앱 안에서 인터뷰 세션을 다시 열어 설계를 고치고
재출력하는 경로는 제품에 두지 않는다 — 설계를 더 고치고 싶으면 연결된 Codex/Claude
Code가 직접 `.project-intel/`을 고친다. 3.3의 해소 프롬프트가 실제로 이 경로("판단과
실행 모두 옆 agent가 한다")를 검증했다. 완전히 다른 앱을 만들고 싶으면 새 프로젝트
디렉터리를 새로 지정한다.

> 이 절은 `docs/product_flow_decisions.md`에서 확정한 내용을 정리한 것이다. 그 문서에는
> 이 결정에 이른 대화의 맥락과, 아직 정하지 않은 지점(Wiki가 분석할 세션을 사용자가
> 고를지 앱이 찾을지 등)이 남아 있다.
...
처음부터 모든 기능을 완성하지 않고, §3 서두의 두 진입 경로 중 **"새 앱을 만드는 경우"**
하나를 끝까지 완성한다. 시각화(3.2)·기술부채(3.4)나 "이미 코드가 있는 프로젝트" 경로는
이 범위 밖이다.
