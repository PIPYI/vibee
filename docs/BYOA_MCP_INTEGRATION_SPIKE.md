...
이 spike에 있는 `mode: "task"`는 예외가 아니라 **검증 장치**다. Phase A/B가 Agent Control
채널이 살아 있는지 — 브라우저 프롬프트가 로컬 agent의 turn이 되고 agent가 지정된 디렉터리에서
실제로 파일을 고치는지 — 를 확인하려고 만든 것이고, acceptance가 계속 그것을 재고 있다.
**제품 경로가 아니며 그 위에 기능을 쌓지 않는다.** 이 구분을 놓치면 §1.2가 코드에서 조용히
뒤집힌다.
