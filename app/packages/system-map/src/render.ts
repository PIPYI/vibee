...
export function renderSystemMapSvg(doc: SystemMapDocument, options?: RenderOptions): string {
  const audience: SystemMapAudience = options?.audience ?? "simple";
  ...
  const layout = calculateSystemMapLayout(projected);
...
function accentColorDark(type: SystemMapComponentType): string {
  const colors: Record<SystemMapComponentType, string> = {
    frontend: "#22d3ee",
    backend: "#a78bfa",
    database: "#34d399",
...
function renderBoundary(
  ...
  // 경계 박스가 캔버스 최상단(y가 0에 가까움)에 놓이면 라벨을 박스 위 8px에 그리는 기본
  // 위치가 뷰포트 밖(음수 y)으로 밀려 잘려 보인다 — 최소 12px 아래로 고정해 항상 보이게 한다
  // (여러 실제 프로젝트로 검증하다 발견한 렌더링 버그, geometry.ts의 레이아웃 로직은 그대로 두고
  // 라벨 위치만 방어적으로 clamp한다).
  const labelY = Math.max(minY - 8, 12);
  const badgeEl =
    boundary.kind === "runtime"
      ? `<text class="av-boundary-badge" x="${minX + 10}" y="${labelY}">${runtimeBadge}</text>`
      : "";
...
function renderComponent(c: SystemMapComponent, rect: Rect, audience: SystemMapAudience, selectedSemanticRef?: string): string {
...
function renderLegend(usedTypes: SystemMapComponentType[], vbW: number, vbH: number, audience: SystemMapAudience): string {
  const swatchSize = 10;
  const rowHeight = 18;
