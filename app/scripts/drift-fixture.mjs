 ...
 * 드리프트 리뷰 검증용 fixture.
 *
 * **인계가 이미 끝난 프로젝트**를 만든다 — `design.json`과 하네스(`AGENTS.md`/`CLAUDE.md`)가
 * 루트에 놓여 있고, 그 위에 커밋이 쌓인 상태. 설계는 손으로 써 둔다. 인터뷰가 실제로 이런
 * 초안을 만들어낸다는 것은 Design 기능에서 이미 확인했고, 여기서 재고 싶은 것은 그 다음 —
 * **저장된 결정이 코드 변경을 판정하는 기준으로 쓰이는가** — 이므로 입력을 고정해야
 * 실행마다 비교할 수 있다.
 */
...
import { appRoot } from "./_shared.mjs";

export const driftFixtureDir = join(appRoot, "tmp", "drift-fixture");

const HARNESS_MARKER = "<!-- vci:generated -->";
  ...
  entities: [
    {
      id: "ENT-1",
      name: "물건",
      note: "주인이 등록한, 빌려줄 수 있는 물건",
      relations: ["ACTOR-1이 소유한다"],
      states: ["대여 가능", "대여 중"],
      source: "user",
    },
    {
      id: "ENT-2",
      name: "대여",
      note: "빌리는 사람이 물건에 대해 보낸 신청 한 건",
      relations: ["ENT-1에 걸린다"],
      states: ["신청됨", "수락됨", "반납됨"],
      source: "ai",
    },
    {
      id: "ENT-3",
      name: "후기",
      note: "대여가 끝난 뒤 남기는 한마디",
      relations: ["ENT-2에 달린다"],
      states: ["작성됨"],
      source: "user",
    },
  ],
...
export function createDriftFixture(dir = driftFixtureDir) {
  ...
  write(dir, "design.json", JSON.stringify(DESIGN, null, 2));
  write(dir, "app_design.md", APP_DESIGN);
  write(dir, "AGENTS.md", HARNESS);
  write(dir, "CLAUDE.md", HARNESS);

  git(dir, ["init", "-q", "-b", "main"]);
  git(dir, ["config", "user.email", "vci-app@example.invalid"]);
  git(dir, ["config", "user.name", "VCI Drift Fixture"]);
  git(dir, ["add", "-A"]);
...
/**
 * 리뷰 turn이 **코드**를 건드렸는지. 루트의 생성 파일(`design.json`/`reviews.json`/
 * `architecture.{json,md}`)과 `wiki/`는 제외한다 — 거기에 리뷰 기록을 남기는 것은 우리
 * bridge가 하는 정상 동작이지 agent가 코드를 고친 것이 아니다.
 */
const GENERATED_ROOT_FILES = ["design.json", "reviews.json", "architecture.json", "architecture.md"];
...
export function isCodeClean(dir) {
  return git(dir, ["status", "--porcelain"])
    ...
    .every((line) => {
      ...
      return GENERATED_ROOT_FILES.includes(path) || path.startsWith("wiki/");
    });
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // 핸드오프 커밋 하나만으로는 리뷰할 게 없다 — 위반 커밋 하나(DEC-1을 어김)와 무해 커밋
  // 하나(일반적인 코드 냄새만 있고 아무 기준도 안 어김)를 바로 쌓아서, Design 탭이 아니라
  // 이 fixture 경로로 Drift를 바로 눌러볼 수 있게 한다.
  commitViolation(dir);
  commitBenign(dir);
  ...
  console.log("  커밋 3개: 인계 → DEC-1 위반(결제 끼워넣기) → 무해(코드 정리)");
  console.log("  기대 결과: 위반 커밋에서만 DEC-1 finding 하나, 무해 커밋에는 아무것도 없어야 함");
  if (!existsSync(join(dir, "design.json"))) throw new Error("design.json이 만들어지지 않았습니다");
}
