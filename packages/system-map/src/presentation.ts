...
function resolveLabel(entity: { label: string; presentation?: AudiencePresentation }, audience: SystemMapAudience): string {
 ...
 */
function resolveSublabel(entity: Presentable, audience: SystemMapAudience): string | undefined {
...
export function applyAudiencePresentation(
  ...
  const components: SystemMapComponent[] = document.components.map((c) => {
  ...
  const boundaries: SystemMapBoundary[] = document.boundaries.map((b) => ({
    ...b,
    label: resolveLabel(b, audience),
  }));
  const connections: SystemMapConnection[] = document.connections.map((conn) => {
