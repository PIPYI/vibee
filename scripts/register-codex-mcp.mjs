#!/usr/bin/env node
/**
 * 이 앱의 MCP server를 사용자의 Codex CLI에 등록한다.
 *
 * 전역 Codex 설정을 건드리는 것은 이 스크립트가 유일하며, `vci-app`이라는 이름의
 * 항목 하나만 추가한다. `npm run mcp:unregister`는 정확히 그 항목만 제거한다.
 * 사용자의 다른 설정은 읽지도, 덮어쓰지도, 초기화하지도 않는다.
 *
 * 다음 명령과 동등하다.
 *   codex mcp add vci-app --env BRIDGE_URL=... --env BRIDGE_TOKEN=... \
 *     -- node <app>/packages/mcp-server/dist/index.js
 */
...
console.log(`\nDone. Verify with \`codex mcp list\`, remove with \`npm run mcp:unregister\`.`);
