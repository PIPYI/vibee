 ...
 *
 * 인계가 끝난 프로젝트를 흉내내므로 `design.json`이 이미 있다(프로젝트 루트, 다른
 * 인계 산출물과 같은 레벨 — `.project-intel/`은 더 이상 쓰지 않는다).
 */
...
export const architectureFixtureDir = join(appRoot, "tmp", "architecture-fixture");
/** design.json 없이 "기존 코드베이스를 여는" 진입 경로를 흉내내는 변형. */
export const architectureNoDesignFixtureDir = join(appRoot, "tmp", "architecture-fixture-no-design");
...
`,
  "design.json": JSON.stringify(DESIGN, null, 2) + "\n",
};
 ...
 *
 * `withDesign: false`면 `design.json`을 빼서, 인터뷰를 거치지 않고 "기존 코드베이스를
 * 여는" 진입 경로(`docs/product_flow_decisions.md` "프로젝트 진입 경로")를 흉내낸다 —
 * oversized-module이 REQ/ENTITY 없이 코드만 보고도 판단되는지 보는 데 쓴다.
 */
export function createArchitectureFixture(dir = architectureFixtureDir, { withDesign = true } = {}) {
  ...
  for (const [path, content] of Object.entries(BASE_FILES)) {
    if (!withDesign && path === "design.json") continue;
    write(dir, path, content);
  }

  git(dir, ["init", "-q", "-b", "main"]);
  git(dir, ["config", "user.email", "vci-app@example.invalid"]);
  git(dir, ["config", "user.name", "VCI Architecture Fixture"]);
  git(dir, ["add", "-A"]);
...
/**
 * 구조 점검이 **소스나 설계를** 건드렸는지. 루트의 `architecture.json`/`architecture.md`에
...
export function isSourceClean(dir) {
  return git(dir, ["status", "--porcelain", "--untracked-files=all"])
    ...
    .every((path) => path === "architecture.json" || path === "architecture.md");
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const dir = createArchitectureFixture();
  console.log(`Architecture fixture ready at ${dir}`);
  console.log("  검출 대상 3개: 파일 비대화(src/app.js) · 의미 중복(member.js/borrower.js) · 방치된 TODO(store.js)");
